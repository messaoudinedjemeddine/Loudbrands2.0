...
const getBackendBase = (requestOrigin: string) => {
  ...
  // If API URL is same as frontend (wrong config), use default backend so proxy works
...
async function proxyRequest(
  ...
  try {
    ...
    const url = `${backendBase}/api/shipping${pathWithLeading}${request.nextUrl.search}`;
    ...
    return NextResponse.json(
      { error: 'Failed to reach shipping API. Ensure BACKEND_URL or NEXT_PUBLIC_API_URL points to your backend.' },
      { status: 502 }
    );
  }
}
