...
router.post('/ship-order/:orderId', async (req, res) => {
  try {
    ...
    const result = await yalidineService.createParcel(yalidineService.formatParcelData({
      ...payload,
      orderId: payload.order_id,
      contactPhone: payload.contact_phone,
      fromWilayaName: payload.from_wilaya_name,
      toWilayaName: payload.to_wilaya_name,
      toCommuneName: payload.to_commune_name,
      productList: payload.product_list,
      stopDeskId: payload.stopdesk_id,
      isStopDesk: payload.is_stopdesk,
      declaredValue: payload.declared_value
    }));
    // Wait, formatParcelData expects camelCase inputs and returns snake_case. 
    // My payload above is mixed. Let's use clean object passed to formatParcelData.
    ...
    const apiResponse = await yalidineService.createParcel(yalidineService.formatParcelData({
      ...
      productList,
      price: calculatedSubtotal,
      weight: 1, length: 30, width: 20, height: 10,
